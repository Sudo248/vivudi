...
class HotelRepository{
  ...
  Future<void> setHeader() async{
    networkService.headers["Authorization"] = "Bearer ${(await SharedPreferences.getInstance()).getString(Constant.token)}";
  }

  Future<HotelResponse> createHotel(HotelRequest request) async{
    try{
      final response = await networkService.post(Constant.hotelUrl, data: request.toMap());
  ...
  Future<HotelResponse> getHotel(String id) async{
    try{
      final response = await networkService.get(Constant.hotelUrl, queryParameters: {"hotelId": id});
      final hotelResponse = HotelResponse.fromMap(response.data);
      return hotelResponse;
    }catch(e){
      return HotelResponse(success: false, error: e.toString());
  ...
  Future<HotelResponse> updateHotel(String id, HotelRequest request) async{
    try{
      final response = await networkService.put(Constant.hotelUrl, data: request.toMap(),queryParameters: {"hotelId": id});
      final hotelResponse = HotelResponse.fromMap(response.data);
      return hotelResponse;
    }catch(e){
      return HotelResponse(success: false, error: e.toString());
  ...
  Future<HotelResponse> deleteHotel(String id) async{
    try{
      final response = await networkService.delete(Constant.hotelUrl, queryParameters: {"hotelId": id});
      final hotelResponse = HotelResponse.fromMap(response.data);
      return hotelResponse;
    }catch(e){
      return HotelResponse(success: false, error: e.toString());
